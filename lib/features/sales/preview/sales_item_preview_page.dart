...
import 'package:farm/features/sales/preview/bloc/sales_item_preview_bloc.dart';
...
@RoutePage()
class SalesItemPreviewPage extends StatefulWidget {
  const SalesItemPreviewPage({super.key, required this.item, this.connection});

  final BluetoothConnection? connection;
  final Sales item;

  @override
  State<SalesItemPreviewPage> createState() => _SalesItemPreviewPageState();
}
...
class _SalesItemPreviewPageState
  ...
  Widget buildPageListeners({required Widget child}) {
    return MultiBlocListener(
      listeners: [
        BlocListener<SalesItemPreviewBloc, SalesItemPreviewState>(
          ...
          listener: (context, state) async {
            ...
            if (state.rfid.isNotEmpty &&
                currentRoute == 'SalesItemPreviewRoute') {
  ...
  Widget buildPage(BuildContext context) {
    return PopScope(
      ...
      child: MultiBlocProvider(
        providers: [
          BlocProvider<SalesItemPreviewBloc>(
  ...
  Widget _emptyState(SalesItemPreviewState state, double width, bool isTablet) {
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Text(
            'Tambah Item Penjualan #${(widget.item.customer_detail?.name).defaultValue('-')}',
  ...
  Widget _received(SalesItemPreviewState state, double width, bool isTablet) {
    return Column(
      mainAxisSize: !isTablet ? MainAxisSize.min : MainAxisSize.max,
      children: [
        SizedBox(height: isTablet ? Dimens.d64 : 0),
  ...
  Widget _contentView(double width, bool isTablet) {
    return Container(
      ...
      child: BlocProvider.value(
        value: bloc,
        child: BlocBuilder<SalesItemPreviewBloc, SalesItemPreviewState>(
